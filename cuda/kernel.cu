...
int main()
{
	// Print matrix size
	std::cout << "===================================================================" << endl;
	cout << "Processing matrix " << MATRIXWIDTH << " x " << MATRIXWIDTH << endl;
...
void InverseRadonTransformCuda(Mat & im_in, Mat & im_out)
{
	...
	for (int i = 0; i < im_in.rows; i++)
	{
		...
		// Kernel matrix add
		MatAdd << <MATRIXWIDTH, MATRIXWIDTH >> > (d_A, d_B, d_C, m, n);
...
void PerformRadonAndInverseRadon(string method)
{
	...
	// Init vars
	Mat sinogram_display, sinogram_filtered, img_reconstructed;
	...
	else  											// cuda without shared memory
...
void ApplyFilterCuda(Mat & im_in, string filter_method, Mat & im_out)
{
	...
	MultiplySpectrums(planes[0], planes[1], planes_filter[0], planes_filter[1], img_complex);
